...
def parse_model(d, ch, verbose=True):
    ...
    if scales:
        scale = d.get("scale")
        if not scale: scale = tuple(scales.keys())[0]
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in {Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF,
                 C2fPSA, C2PSA, DWConv, Focus, BottleneckCSP, C1, C2, C2f, C3k2,
                 RepNCSPELAN4, ELAN1, ADown, AConv, SPPELAN, C2fAttn, C3, C3TR,
                 C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown,
                 C2fCIB, SimAM}:
            c1, c2 = ch[f], args[0]
            if c2 != nc: c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C3k2, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fPSA, C2fCIB, C2PSA}:
                args.insert(2, n);
                n = 1

        # elif m is BiFPN_RepC3:
        elif m in (BiFPN_RepC3, BiFPN_Ablation):
            c1 = [ch[x] for x in f]
            # args[0] 是 YAML 里的输出通道列表，这里进行缩放
            c2_list = [make_divisible(min(x, max_channels) * width, 8) for x in args[0]]
            m_instance = m(c1, c2_list)
            for idx, out_ch in enumerate(c2_list):
                sub_m = SubOut(m_instance, idx)
                sub_m.i, sub_m.f, sub_m.type = i, f, f"BiFPN_P{idx + 3}"
                layers.append(sub_m)
                ch.append(out_ch)
